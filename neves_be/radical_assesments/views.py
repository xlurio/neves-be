...
from neves_be.radical_assesments.serializers import RadicalSessionAssessmentSerializer
from neves_be.radical_assesments.services import ANSWER_CHOICES
from neves_be.radical_assesments.services import create_session_assessment
from neves_be.radical_assesments.services import owned_assessment_or_404
from neves_be.radical_assesments.services import serialize_question_payload
from neves_be.radical_assesments.services import serialize_result_questions
...
def radical_session_assessments_view(request: Request, session_id: SessionId) -> Response:
    ...
    if request.method == "GET":
        ...
        return paginator.get_paginated_response(
            RadicalSessionAssessmentSerializer(page, many=True).data,
        )
    ...
    return Response(
        RadicalSessionAssessmentSerializer(test).data,
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def radical_assessment_question_view(
    request: Request,
...
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def radical_assessment_answer_view(request: Request, assessment_id: RadicalSessionAssessmentId) -> Response:
    test = owned_assessment_or_404(request, assessment_id)
...
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def radical_assessment_finish_view(request: Request, assessment_id: RadicalSessionAssessmentId) -> Response:
    test = owned_assessment_or_404(request, assessment_id)
    unanswered_question = (
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def radical_assessment_result_view(request: Request, assessment_id: RadicalSessionAssessmentId) -> Response:
    test = owned_assessment_or_404(request, assessment_id)
